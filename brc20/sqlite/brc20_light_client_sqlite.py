...
INDEXER_VERSION = "opi-brc20-light-client-sqlite v0.3.1"
DB_VERSION = 5
...
def deploy_inscribe(block_height, inscription_id, deployer_pkScript, deployer_wallet, tick, original_tick, max_supply, decimals, limit_per_mint, is_self_mint):
  ...
  execute_with_params('''insert into brc20_tickers (tick, original_tick, max_supply, decimals, limit_per_mint, remaining_supply, block_height, is_self_mint, deploy_inscription_id)
    values (%s, %s, %s, %s, %s, %s, %s, %s, %s);''', (tick, original_tick, str(max_supply), decimals, str(limit_per_mint), str(max_supply), block_height, is_self_mint, inscription_id))
  ...
  while len(inner_event_providers) != 0:
    ...
    try:
      ...
      if int(r.text) not in EVENT_PROVIDER_USABLE_EVENT_HASH_VERSIONS:
        ...
        print("Usable event hash versions: " + str(EVENT_PROVIDER_USABLE_EVENT_HASH_VERSIONS))
        inner_event_providers.remove(event_provider)
        time.sleep(0.1)
        continue
    except:
...
def index_block(block_height):
  ...
  ticks = {}
  for t in ticks_:
    ticks[t[0]] = [int(t[1]), int(t[2]), int(t[3]), t[4] == 'true', t[5]]
